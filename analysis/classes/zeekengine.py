...
class ZeekEngine(object):
    ...
    def __init__(self, capture_directory):
        ...
        with open(
            os.path.join(
                ...
                "locales/{}.json".format(self.userlang),
            )
        ) as f:
    ...
    def fill_dns(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "dns.log")):
            for record in ParseZeekLogs(
                os.path.join(dir, "dns.log"), output_format="json", safe_headers=False
            ):
    ...
    def netflow_check(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "conn.log")):
            for record in ParseZeekLogs(
                os.path.join(dir, "conn.log"), output_format="json", safe_headers=False
            ):
                if record is not None:
                    c = {
        ...
        if self.whitelist_analysis:
            ...
            for i, c in enumerate(self.conns):
                if c["ip_dst"] in [ip for ip in self.wl_hosts]:
                    ...
                    c["resolution"].endswith("." + dom) for dom in self.wl_domains
                ]:
                    self.whitelist.append(self.conns[i])
                    self.conns[i] = False
        ...
        if self.heuristics_analysis:
            for c in self.conns:
                ...
                if c["proto"] in ["UDP", "ICMP"]:
                    ...
                    self.alerts.append(
                        {
                            "title": self.template["PROTO-01"]["title"].format(
                                c["proto"].upper(), c["resolution"]
                            ),
                            "description": self.template["PROTO-01"][
                                "description"
                ...
                if c["port_dst"] >= max_ports:
                    ...
                    self.alerts.append(
                        {
                            "title": self.template["PROTO-02"]["title"].format(
                                c["proto"].upper(), c["resolution"], max_ports
                            ),
                            "description": self.template["PROTO-02"][
                                "description"
                            ].format(
                                c["proto"].upper(), c["resolution"], c["port_dst"]
                            ),
                            "host": c["resolution"],
                            "level": "Low",
                            "id": "PROTO-02",
                ...
                if c["service"] == "http" and c["port_dst"] == http_default_port:
                    ...
                    self.alerts.append(
                        {
                            "title": self.template["PROTO-03"]["title"].format(
                                c["resolution"]
                            ),
                            "description": self.template["PROTO-03"][
                                "description"
                            ].format(c["resolution"]),
                            "host": c["resolution"],
                            "level": "Low",
                ...
                if c["service"] == "http" and c["port_dst"] != http_default_port:
                    ...
                    self.alerts.append(
                        {
                            "title": self.template["PROTO-04"]["title"].format(
                                c["resolution"], c["port_dst"]
                            ),
                            "description": self.template["PROTO-04"][
                                "description"
                ...
                if c["ip_dst"] == c["resolution"]:
                    ...
                    self.alerts.append(
                        {
                            "title": self.template["PROTO-05"]["title"].format(
                                c["ip_dst"]
                            ),
                            "description": self.template["PROTO-05"][
                                "description"
                            ].format(c["ip_dst"]),
                            "host": c["ip_dst"],
                            "level": "Low",
            ...
            for c in self.conns:
                ...
                for host in self.bl_hosts:
                    if c["ip_dst"] == host[0]:
                        ...
                        self.alerts.append(
                            {
                                "title": self.template["IOC-01"]["title"].format(
                                    c["resolution"], c["ip_dst"], host[1].upper()
                                ),
                                "description": self.template["IOC-01"][
                                    "description"
                                ].format(c["ip_dst"]),
                                "host": c["resolution"],
                                "level": "High",
                ...
                for cidr in self.bl_cidrs:
                    if IPAddress(c["ip_dst"]) in cidr[0]:
                        ...
                        self.alerts.append(
                            {
                                "title": self.template["IOC-02"]["title"].format(
                                    c["resolution"], cidr[0], cidr[1].upper()
                                ),
                                "description": self.template["IOC-02"][
                                    "description"
                                ].format(c["resolution"]),
                                "host": c["resolution"],
                                "level": "Moderate",
                ...
                for domain in self.bl_domains:
                    if c["resolution"].endswith(domain[0]):
                        if domain[1] != "tracker":
                            ...
                            self.alerts.append(
                                {
                                    "title": self.template["IOC-03"]["title"].format(
                                        c["resolution"], domain[1].upper()
                                    ),
                                    "description": self.template["IOC-03"][
                                        "description"
                                    ].format(c["resolution"]),
                                    "host": c["resolution"],
                                    "level": "High",
                            ...
                            self.alerts.append(
                                {
                                    "title": self.template["IOC-04"]["title"].format(
                                        c["resolution"], domain[1].upper()
                                    ),
                                    "description": self.template["IOC-04"][
                                        "description"
                                    ].format(c["resolution"]),
                                    "host": c["resolution"],
                                    "level": "Moderate",
                ...
                for domain in self.bl_freedns:
                    if c["resolution"].endswith("." + domain[0]):
                        ...
                        self.alerts.append(
                            {
                                "title": self.template["IOC-05"]["title"].format(
                                    c["resolution"]
                                ),
                                "description": self.template["IOC-05"][
                                    "description"
                                ].format(c["resolution"]),
                                "host": c["resolution"],
                                "level": "Moderate",
                ...
                for tld in self.bl_tlds:
                    if c["resolution"].endswith(tld[0]):
                        ...
                        self.alerts.append(
                            {
                                "title": self.template["IOC-06"]["title"].format(
                                    c["resolution"]
                                ),
                                "description": self.template["IOC-06"][
                                    "description"
        ...
        if self.active_analysis:
            for c in self.conns:
                    ...
                    if len(name_servers):
                        for ns in self.bl_nameservers:
                            if name_servers[0].endswith(".{}.".format(ns[0])):
                                ...
                                self.alerts.append(
                                    {
                                        "title": self.template["ACT-01"][
                                            "title"
                                        ].format(c["resolution"], name_servers[0]),
                                        "description": self.template["ACT-01"][
                                            "description"
                                        ].format(c["resolution"]),
                                        "host": c["resolution"],
                                        "level": "Moderate",
                ...
                try:  # Domain history check.
                    ...
                    creation_date = (
                        whois_record.creation_date
                        if type(whois_record.creation_date) is not list
                        else whois_record.creation_date[0]
                    )
                    ...
                    if creation_days < 365:
                        ...
                        self.alerts.append(
                            {
                                "title": self.template["ACT-02"]["title"].format(
                                    c["resolution"], creation_days
                                ),
                                "description": self.template["ACT-02"][
                                    "description"
                                ].format(c["resolution"]),
                                "host": c["resolution"],
                                "level": "Moderate",
    ...
    def files_check(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "files.log")):
            for record in ParseZeekLogs(
                os.path.join(dir, "files.log"), output_format="json", safe_headers=False
            ):
                if record is not None:
                    f = {
            ...
            if f["mime_type"] == "application/x-x509-user-cert":
                for cert in bl_certs:  # Check for blacklisted certificate.
                    if f["sha1"] == cert[0]:
                        ...
                        self.alerts.append(
                            {
                                "title": self.template["IOC-07"]["title"].format(
                                    cert[1].upper(), host
                                ),
                                "description": self.template["IOC-07"][
                                    "description"
                                ].format(f["sha1"], host),
                                "host": host,
                                "level": "High",
    ...
    def http_check(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "http.log")):
            for record in ParseZeekLogs(
                os.path.join(dir, "http.log"), output_format="json", safe_headers=False
            ):
                if record is not None:
                    c = {"host": record["host"]}
            ...
            for c in self.http:
                ...
                for h in self.bl_domains:
                    if h[1] != "tracker":
                        if c["host"].endswith(h[0]):
                            self.alerts.append(
                                {
                                    "title": self.template["IOC-08"]["title"].format(
                                        c["host"], h[1].upper()
                                    ),
                                    "description": self.template["IOC-08"][
                                        "description"
                                    ].format(c["host"]),
                                    "host": c["host"],
                                    "level": "High",
                ...
                for h in self.bl_freedns:
                    if c["host"].endswith(h[0]):
                        self.alerts.append(
                            {
                                "title": self.template["IOC-09"]["title"].format(
                                    c["host"]
                                ),
                                "description": self.template["IOC-09"][
                                    "description"
                                ].format(c["host"]),
                                "host": c["host"],
                                "level": "Moderate",
                ...
                for h in self.bl_tlds:
                    if c["host"].endswith(h[0]):
                        self.alerts.append(
                            {
                                "title": self.template["IOC-10"]["title"].format(
                                    c["host"]
                                ),
                                "description": self.template["IOC-10"][
                                    "description"
                                ].format(c["host"], h[0]),
                                "host": c["host"],
                                "level": "Low",
    ...
    def ssl_check(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "ssl.log")):
            for record in ParseZeekLogs(
                os.path.join(dir, "ssl.log"), output_format="json", safe_headers=False
            ):
                if record is not None:
                    c = {
                ...
                for c in self.conns:
                    if host in c["resolution"]:
                        ...
                        if cert["port"] not in ssl_default_ports:
                            ...
                            self.alerts.append(
                                {
                                    "title": self.template["SSL-01"]["title"].format(
                                        cert["port"], host
                                    ),
                                    "description": self.template["SSL-01"][
                                        "description"
                                    ].format(host),
                                    "host": host,
                                    "level": "Moderate",
                        ...
                        if cert["issuer"] in free_issuers:
                            ...
                            self.alerts.append(
                                {
                                    "title": self.template["SSL-02"]["title"].format(
                                        host
                                    ),
                                    "description": self.template["SSL-02"][
                                        "description"
                                    ],
                                    "host": host,
                                    "level": "Moderate",
                        ...
                        if (
                            ...
                            self.alerts.append(
                                {
                                    "title": self.template["SSL-03"]["title"].format(
                                        host
                                    ),
                                    "description": self.template["SSL-03"][
                                        "description"
                                    ].format(host),
                                    "host": host,
                                    "level": "Moderate",
                ...
                for domain in self.bl_domains:
                    if domain[1] != "tracker":
                        if cert["cn"].endswith(domain[0]):
                            self.alerts.append(
                                {
                                    "title": self.template["SSL-04"]["title"].format(
                                        domain[0], domain[1].upper()
                                    ),
                                    "description": self.template["SSL-04"][
                                        "description"
                                    ].format(domain[0]),
                                    "host": domain[0],
                                    "level": "High",
    ...
    def alerts_check(self):
        ...
        for host, nb in hosts.items():
            if nb >= get_config(("analysis", "max_alerts")):
                self.alerts.append(
                    {
                        "title": self.template["ADV-01"]["title"].format(host),
                        "description": self.template["ADV-01"]["description"].format(
                            host, nb
                        ),
                        "host": host,
                        "level": "Moderate",
    ...
    def start_zeek(self):
        ...
        sp.Popen(
            "cd {} && mv *.log assets/".format(self.working_dir), shell=True
        ).wait()
