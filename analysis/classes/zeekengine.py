...
class ZeekEngine(object):
    ...
    def fill_dns(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "dns.log")):
            for record in ParseZeekLogs(os.path.join(dir, "dns.log"),
                                        output_format="json",
                                        safe_headers=False):
        ...
        if self.heuristics_analysis:
            for c in self.conns:
                ...
                if c["proto"] in ["UDP", "ICMP"]:
                    ...
                    self.alerts.append({
                        "title":
                        self.template["PROTO-01"]["title"].format(
                            c["proto"].upper(), c["resolution"]),
                        "description":
                        self.template["PROTO-01"]["description"].format(
                            c["proto"].upper(), c["resolution"]),
                        "host":
                        c["resolution"],
                        "level":
                ...
                if c["port_dst"] >= max_ports:
                    ...
                    self.alerts.append({
                        "title":
                        self.template["PROTO-02"]["title"].format(
                            c["proto"].upper(), c["resolution"], max_ports),
                        "description":
                        self.template["PROTO-02"]["description"].format(
                            c["proto"].upper(), c["resolution"],
                            c["port_dst"]),
                        "host":
                        c["resolution"],
                ...
                if c["service"] == "http" and c[
                    ...
                    self.alerts.append({
                        "title":
                        self.template["PROTO-03"]["title"].format(
                            c["resolution"]),
                        "description":
                        self.template["PROTO-03"]["description"].format(
                            c["resolution"]),
                        "host":
                        c["resolution"],
                        "level":
                ...
                if c["service"] == "http" and c[
                    ...
                    self.alerts.append({
                        "title":
                        self.template["PROTO-04"]["title"].format(
                            c["resolution"], c["port_dst"]),
                        "description":
                        self.template["PROTO-04"]["description"].format(
                            c["resolution"], c["port_dst"]),
                        "host":
                        c["resolution"],
                        "level":
                ...
                if c["ip_dst"] == c["resolution"]:
                    ...
                    self.alerts.append({
                        "title":
                        self.template["PROTO-05"]["title"].format(c["ip_dst"]),
                        "description":
                        self.template["PROTO-05"]["description"].format(
                            c["ip_dst"]),
                        "host":
                        c["ip_dst"],
                        "level":
            ...
            for c in self.conns:
                ...
                for host in self.bl_hosts:
                    if c["ip_dst"] == host[0]:
                        ...
                        self.alerts.append({
                            "title":
                            self.template["IOC-01"]["title"].format(
                                c["resolution"], c["ip_dst"], host[1].upper()),
                            "description":
                            self.template["IOC-01"]["description"].format(
                                c["ip_dst"]),
                            "host":
                            c["resolution"],
                            "level":
                ...
                for cidr in self.bl_cidrs:
                    if IPAddress(c["ip_dst"]) in cidr[0]:
                        ...
                        self.alerts.append({
                            "title":
                            self.template["IOC-02"]["title"].format(
                                c["resolution"], cidr[0], cidr[1].upper()),
                            "description":
                            self.template["IOC-02"]["description"].format(
                                c["resolution"]),
                            "host":
                            c["resolution"],
                            "level":
                ...
                for domain in self.bl_domains:
                    if c["resolution"].endswith(domain[0]):
                        if domain[1] != "tracker":
                            ...
                            self.alerts.append({
                                "title":
                                self.template["IOC-03"]["title"].format(
                                    c["resolution"], domain[1].upper()),
                                "description":
                                self.template["IOC-03"]["description"].format(
                                    c["resolution"]),
                                "host":
                                c["resolution"],
                                "level":
                            ...
                            self.alerts.append({
                                "title":
                                self.template["IOC-04"]["title"].format(
                                    c["resolution"], domain[1].upper()),
                                "description":
                                self.template["IOC-04"]["description"].format(
                                    c["resolution"]),
                                "host":
                                c["resolution"],
                                "level":
                ...
                for domain in self.bl_freedns:
                    if c["resolution"].endswith("." + domain[0]):
                        ...
                        self.alerts.append({
                            "title":
                            self.template["IOC-05"]["title"].format(
                                c["resolution"]),
                            "description":
                            self.template["IOC-05"]["description"].format(
                                c["resolution"]),
                            "host":
                            c["resolution"],
                            "level":
                ...
                for tld in self.bl_tlds:
                    if c["resolution"].endswith(tld[0]):
                        ...
                        self.alerts.append({
                            "title":
                            self.template["IOC-06"]["title"].format(
                                c["resolution"]),
                            "description":
                            self.template["IOC-06"]["description"].format(
                                c["resolution"], tld[0]),
                            "host":
                            c["resolution"],
                            "level":
            ...
            for c in self.conns:
                    ...
                    if len(name_servers):
                        for ns in self.bl_nameservers:
                            if name_servers[0].endswith(".{}.".format(ns[0])):
                                ...
                                self.alerts.append({
                                    "title":
                                    self.template["ACT-01"]["title"].format(
                                        c["resolution"], name_servers[0]),
                                    "description":
                                    self.template["ACT-01"]
                                    ["description"].format(c["resolution"]),
                                    "host":
                                    c["resolution"],
                                    "level":
                                    "Moderate",
                ...
                try:  # Domain history check.
                    ...
                    creation_date = (whois_record.creation_date if type(
                        whois_record.creation_date) is not list else
                                     whois_record.creation_date[0])
                    creation_days = abs((datetime.now() - creation_date).days)
                    if creation_days < 365:
                        c["alert_tiggered"] = True
                        self.alerts.append({
                            "title":
                            self.template["ACT-02"]["title"].format(
                                c["resolution"], creation_days),
                            "description":
                            self.template["ACT-02"]["description"].format(
                                c["resolution"]),
                            "host":
                            c["resolution"],
                            "level":
    ...
    def files_check(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "files.log")):
            for record in ParseZeekLogs(os.path.join(dir, "files.log"),
                                        output_format="json",
                                        safe_headers=False):
                if record is not None:
                    f = {
            ...
            if f["mime_type"] == "application/x-x509-user-cert":
                for cert in bl_certs:  # Check for blacklisted certificate.
                    if f["sha1"] == cert[0]:
                        ...
                        self.alerts.append({
                            "title":
                            self.template["IOC-07"]["title"].format(
                                cert[1].upper(), host),
                            "description":
                            self.template["IOC-07"]["description"].format(
                                f["sha1"], host),
                            "host":
                            host,
                            "level":
    ...
    def http_check(self, dir):
        ...
        if os.path.isfile(os.path.join(dir, "http.log")):
            for record in ParseZeekLogs(os.path.join(dir, "http.log"),
                                        output_format="json",
                                        safe_headers=False):
                if record is not None:
                    c = {"host": record["host"]}
            ...
            for c in self.http:
                ...
                for h in self.bl_domains:
                    if h[1] != "tracker":
                        if c["host"].endswith(h[0]):
                            self.alerts.append({
                                "title":
                                self.template["IOC-08"]["title"].format(
                                    c["host"], h[1].upper()),
                                "description":
                                self.template["IOC-08"]["description"].format(
                                    c["host"]),
                                "host":
                                c["host"],
                                "level":
                ...
                for h in self.bl_freedns:
                    if c["host"].endswith(h[0]):
                        self.alerts.append({
                            "title":
                            self.template["IOC-09"]["title"].format(c["host"]),
                            "description":
                            self.template["IOC-09"]["description"].format(
                                c["host"]),
                            "host":
                            c["host"],
                            "level":
                ...
                for h in self.bl_tlds:
                    if c["host"].endswith(h[0]):
                        self.alerts.append({
                            "title":
                            self.template["IOC-10"]["title"].format(c["host"]),
                            "description":
                            self.template["IOC-10"]["description"].format(
                                c["host"], h[0]),
                            "host":
                            c["host"],
                            "level":
            ...
            for record in ParseZeekLogs(os.path.join(dir, "ssl.log"),
                ...
                if record is not None:
                    c = {
                        ...
                        record["id.resp_p"],
                        "issuer":
                        record["issuer"] if "issuer" in record else "",
                        "validation_status":
                        record["validation_status"],
                        "cn":
                        record["server_name"]
                        if "server_name" in record else "",
                    }
                ...
                for c in self.conns:
                    if host in c["resolution"]:
                        ...
                        if cert["port"] not in ssl_default_ports:
                            ...
                            self.alerts.append({
                                "title":
                                self.template["SSL-01"]["title"].format(
                                    cert["port"], host),
                                "description":
                                self.template["SSL-01"]["description"].format(
                                    host),
                                "host":
                                host,
                                "level":
                        ...
                        if cert["issuer"] in free_issuers:
                            ...
                            self.alerts.append({
                                "title":
                                self.template["SSL-02"]["title"].format(host),
                                "description":
                                self.template["SSL-02"]["description"],
                                "host":
                                host,
                                "level":
                                "Moderate",
                        ...
                        if (cert["validation_status"] ==
                                "self signed certificate in certificate chain"
                            ):
                            c["alert_tiggered"] = True
                            self.alerts.append({
                                "title":
                                self.template["SSL-03"]["title"].format(host),
                                "description":
                                self.template["SSL-03"]["description"].format(
                                    host),
                                "host":
                                host,
                                "level":
                ...
                for domain in self.bl_domains:
                    if domain[1] != "tracker":
                        if cert["cn"].endswith(domain[0]):
                            self.alerts.append({
                                "title":
                                self.template["SSL-04"]["title"].format(
                                    domain[0], domain[1].upper()),
                                "description":
                                self.template["SSL-04"]["description"].format(
                                    domain[0]),
                                "host":
                                domain[0],
                                "level":
    ...
    def alerts_check(self):
        ...
        for host, nb in hosts.items():
            if nb >= get_config(("analysis", "max_alerts")):
                self.alerts.append({
                    "title":
                    self.template["ADV-01"]["title"].format(host),
                    "description":
                    self.template["ADV-01"]["description"].format(host, nb),
                    "host":
                    host,
                    "level":
                    "Moderate",
    ...
    def start_zeek(self):
        ...
        sp.Popen(
            "cd {} && /opt/zeek/bin/zeek -Cr capture.pcap protocols/ssl/validate-certs"
